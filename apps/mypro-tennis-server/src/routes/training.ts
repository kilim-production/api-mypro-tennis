...
import { normalizedPlayerVitals, playerVitalsAfterTraining } from "../services/playerVitals";
...
trainingRouter.get("/", requireAuth, async (request, response) => {
  ...
  return response.json({
    catalog,
    sessions: sessions.map((session) => ({ ...session, gains: decodeJson(session.gains) }))
  });
});
...
trainingRouter.post(
  "/start",
  requireAuth,
  validateBody(trainingStartSchema),
  async (request, response) => {
    ...
    const player = await prisma.player.findUnique({ where: { userId: request.session!.userId } });
    if (!player)
      return response.status(404).json({ message: "Créez votre joueur avant de vous entraîner." });
    const powerLevel = trainingPowerLevel(player.overall);
    const scaledCost = scaleDevelopmentCost(training.cost, powerLevel, 1.16);
    const scaledDurationMinutes = scaleDevelopmentMinutes(
    ...
    try {
      const session = await prisma.$transaction(async (tx) => {
        ...
        await tx.player.update({
          where: { id: player.id },
          data: { budget: { decrement: scaledCost } }
        });
        return created;
      });
...
trainingRouter.post("/:id/complete", requireAuth, async (request, response) => {
  ...
  const session = await prisma.trainingSession.findFirst({
    where: { id: sessionId, playerId: player.id }
  });
  if (!session) return response.status(404).json({ message: "Séance introuvable." });
  if (session.status === "COMPLETED")
    return response.json({ ...session, gains: decodeJson(session.gains) });
  ...
  for (const [key, value] of Object.entries(decodeJson<Record<string, number>>(session.gains))) {
    stats[key as keyof TennisStats] = clamp(
      stats[key as keyof TennisStats] + trainingGain(value, 0, 0, vitals.fatigue)
    );
  }
  const nextVitals = playerVitalsAfterTraining(player, session.fatigue);
  const updated = await prisma.$transaction(async (tx) => {
    ...
    return tx.player.update({
      ...
      data: {
        ...
        fatigue: nextVitals.fatigue,
        energy: nextVitals.energy,
        morale: nextVitals.morale,
